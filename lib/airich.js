...
const { generateWAMessageFromContent, prepareWAMessageMedia } = require('./Utils');
...
class AIRich extends BaseBuilder {
	...
	// ============================================================
	// FIX: addSuggest - pills sekarang bisa ditekan
	// Ditambahkan: send_message, reply_message, action_type
	// ============================================================
	...
	contextInfo(obj) {
		return this.setContextInfo(obj);
	}

	payload(obj) {
		return this.addPayload(obj);
	}
	...
	code(language, code) {
		return this.addCode(language, code);
	}
	...
	source(sources = []) {
		return this.addSource(sources);
	}

	reels(reelsItems = []) {
		return this.addReels(reelsItems);
	}
	...
	suggest(suggestion) {
		return this.addSuggest(suggestion);
	}

	// ============================================================
	// FIX: build - botJid sekarang dinamis (tidak hardcode '0@bot')
	// Pakai conn.user.id dari handler agar WA kenali sebagai bot valid
	// ============================================================
	build({ forwarded = true, includesUnifiedResponse = true, includesSubmessages = true, quoted, quotedParticipant, botJid = '0@bot', ...options } = {}) {
	...
	// ============================================================
	// FIX: send - teruskan botJid ke build()
	// ============================================================
	async send(jid, { forwarded, includesUnifiedResponse, includesSubmessages, botJid, ...options } = {}) {
		if (!this.#client) throw new Error('Socket is required to send AIRich message');

		const msg = this.build({ forwarded, includesUnifiedResponse, includesSubmessages, botJid, ...options });
...
function installNixcode(sock) {
	if (!sock || typeof sock !== 'object') return sock;

	sock.NIXCODE = { VERSION, Button, ButtonV2, Carousel, AIRich };
	sock.AIRich = AIRich;
	sock.airich = () => new AIRich(sock);
	sock.nixButton = (data = {}) => createButton(sock, data);
	sock.nixButtonV2 = (data = {}) => createButtonV2(sock, data);
	sock.nixCarousel = () => new Carousel(sock);
	sock.nixAIRich = () => new AIRich(sock);

	sock.sendNixButton = async function (jid, data = {}, quoted = null, options = {}) {
		return await createButton(sock, data).send(jid, { quoted, ...options });
	};

	sock.sendNixButtonV2 = async function (jid, data = {}, quoted = null, options = {}) {
		return await createButtonV2(sock, data).send(jid, { quoted, ...options });
	};

	sock.sendNixCarousel = async function (jid, data = {}, quoted = null, options = {}) {
		const carousel = applyBase(new Carousel(sock), data);
		for (const card of data.cards || []) carousel.addCard(await createCard(sock, card));
		return await carousel.send(jid, { quoted, ...options });
	};

	sock.sendAIRich = async function (jid, data = {}, quoted = null, options = {}) {
		const rich = new AIRich(sock);
		if (data.title) rich.setTitle(String(data.title));
		if (data.footer) rich.setFooter(String(data.footer));
		if (data.contextInfo) rich.setContextInfo(data.contextInfo);
		if (data.extraPayload) rich.addPayload(data.extraPayload);
		if (data.tip) rich.addTip(String(data.tip));
		if (Array.isArray(data.suggest) || typeof data.suggest === 'string') rich.addSuggest(data.suggest);
		if (data.text) rich.addText(String(data.text), data.textOptions || {});
		if (data.code) rich.addCode(data.language || 'javascript', String(data.code));
		if (Array.isArray(data.table) && data.table.length) rich.addTable(data.table);
		if (Array.isArray(data.sources) && data.sources.length) rich.addSource(data.sources);
		if (Array.isArray(data.reels) && data.reels.length) rich.addReels(data.reels);
		if (data.image) rich.addImage(data.image);
		if (data.video) rich.addVideo(data.video);
		if (data.product) rich.addProduct(data.product);
		if (data.post) rich.addPost(data.post);

		return await rich.send(jid, {
			forwarded: data.forwarded ?? true,
			includesUnifiedResponse: data.includesUnifiedResponse ?? true,
			includesSubmessages: data.includesSubmessages ?? true,
	...
	sock.sendRich = sock.sendAIRich;
	sock.sendRitch = sock.sendAIRich;

	return sock;
}

const installAIRich = installNixcode;
