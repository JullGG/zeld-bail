...
const makeSocket = (config) => {
    ...
    // Renvy Bail v7: query anti crash + safe retry
    const RENVY_ANTI_CRASH = config.antiCrash !== false;
    // Renvy Bail v11 Ultra Fast: query retry OFF by default to avoid delayed responses.
    const QUERY_RETRY_COUNT = Math.max(0, Number(config.queryRetryCount ?? 0));
    const QUERY_RETRY_BASE_DELAY = Math.max(0, Number(config.queryRetryBaseDelayMs ?? 0));
    const QUERY_RETRY_MAX_DELAY = Math.max(QUERY_RETRY_BASE_DELAY, Number(config.queryRetryMaxDelayMs ?? 1000));
    ...
    const isRenvySafeSocketError = (error) => {
    ...
    const makeSafeQueryNode = (node, msgId) => ({
        tag: node?.tag || 'iq',
        attrs: { ...(node?.attrs || {}), id: msgId, type: 'result' },
        content: []
    });
    ...
    const query = async (node, timeoutMs) => {
        ...
        let lastError;
        for (let attempt = 0; attempt <= QUERY_RETRY_COUNT; attempt++) {
            try {
                ...
                lastError = error;
                if (!RENVY_ANTI_CRASH || !isRenvySafeSocketError(error) || attempt >= QUERY_RETRY_COUNT) break;
                const delay = Math.min(QUERY_RETRY_MAX_DELAY, QUERY_RETRY_BASE_DELAY * Math.pow(2, attempt) + Math.floor(Math.random() * 400));
                logger?.warn?.({ tag: node?.tag, to: node?.attrs?.to, type: node?.attrs?.type, attempt: attempt + 1, retryInMs: delay, error: error?.message || String(error) }, 'Renvy query failed, retrying');
                await sleep(delay);
            }
        }
        if (RENVY_ANTI_CRASH && isRenvySafeSocketError(lastError)) {
            logger?.warn?.({ tag: node?.tag, to: node?.attrs?.to, type: node?.attrs?.type, error: lastError?.message || String(lastError) }, 'Renvy query anti-crash returned safe empty node');
            return makeSafeQueryNode(node, msgId);
        }
        throw lastError;
    };
