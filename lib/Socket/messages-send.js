...
const makeMessagesSocket = (config) => {
    ...
    const deviceToJid = (device, fallbackServer = 's.whatsapp.net') => {
        ...
        return WABinary_1.jidEncode(device.user, fallbackServer, device.device);
    };
    ...
    const FAST_NO_SESSION_SKIP = config.fastNoSessionSkip !== false;
    // Renvy Bail Fast Plus v14: group send tidak menunggu groupMetadata/USync berat.
    // Set renvyFastGroupSend: false kalau ingin mode normal Baileys.
    const RENVY_FAST_GROUP_SEND = config.renvyFastGroupSend !== false;
    ...
    const rawRelayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, AI = true } = {}) => {
                    ...
                    (async () => {
                        ...
                        if (useCachedGroupMetadata && cachedGroupMetadata) {
                            groupData = RENVY_FAST_GROUP_SEND
                                ? await quickValue(cachedGroupMetadata(jid), RENVY_FAST_GROUP_CACHE_TIMEOUT)
