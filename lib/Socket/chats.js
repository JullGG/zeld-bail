...
const makeChatsSocket = (config) => {
    ...
    const onWhatsApp = async (...jids) => {
        ...
        // Renvy Bail v24: jangan crash kalau onWhatsApp dipanggil dengan array/object.
        // Beberapa bot lama kadang mengirim payload seperti { jid }, { id }, atau array jids.
        const normalizeOnWhatsAppJid = (value) => {
            if (!value) {
                return null;
            }
            ...
            if (typeof value === 'object') {
                return value.jid || value.id || value.lid || value.phone || value.user || value.remoteJid || value.key?.remoteJid || null;
            }
            return String(value);
        };
        const inputJids = jids.flat ? jids.flat(Infinity) : [].concat(...jids);
        let addedUsers = 0;

        for (const rawJid of inputJids) {
            const normalizedJid = normalizeOnWhatsAppJid(rawJid);
            if (!normalizedJid) {
                continue;
            }
            const jidText = String(normalizedJid);
            const userPart = jidText.replace('+', '').split('@')[0].split(':')[0].replace(/\D/g, '');
            if (!userPart) {
                logger?.trace?.({ rawJid }, 'Renvy onWhatsApp skipped invalid jid');
                continue;
            }
