...
const makeMessagesRecvSocket = (config) => {
    ...
    const getRetryParticipant = (node) => {
        var _a;
        try {
            const decoded = (0, Utils_1.decodeMessageNode)(node, ((_a = authState.creds.me) === null || _a === void 0 ? void 0 : _a.id) || '', (authState.creds.me && authState.creds.me.lid) || '');
            return decoded.author || node.attrs.participant || node.attrs.from;
        }
        catch (_) {
            return node.attrs.participant || node.attrs.from;
        }
    };
    const clearSignalSession = async (jid, reason, meta = {}) => {
        if (!jid || !signalRepository || typeof signalRepository.jidToSignalProtocolAddress !== 'function') {
            return false;
        }
        try {
            const sessionId = signalRepository.jidToSignalProtocolAddress(jid);
            await authState.keys.set({ session: { [sessionId]: null } });
            logger.warn({ jid, sessionId, reason, ...meta }, 'cleared stale signal session');
            return true;
        }
        catch (error) {
            logger.warn({ error, jid, reason, ...meta }, 'failed to clear stale signal session');
            return false;
        }
    };
    ...
    const sendRetryRequest = async (node, forceIncludeKeys = false) => {
        ...
        if (retryCount > 1) {
            const retryJid = getRetryParticipant(node);
            const cleared = await clearSignalSession(retryJid, 'retrying failed incoming decrypt', { retryCount, msgId });
            if (cleared) {
    ...
    const sendMessagesAgain = async (key, ids, retryNode, receiptNode) => {
        ...
        const retryCount = +((retryNode && retryNode.attrs && retryNode.attrs.count) || 1);
        let injectedFromBundle = false;
        const bundle = receiptNode && (0, Utils_1.extractE2ESessionFromRetryReceipt)(receiptNode);
        if (bundle) {
            try {
        ...
        if (!injectedFromBundle && retryCount > 1) {
            await clearSignalSession(participant, 'outgoing retry without usable key bundle', { retryCount, msgId: ids[0] });
        }
        ...
        logger.debug({ participant, sendToAll, retryCount, injectedFromBundle }, 'prepared session for retry recp');
