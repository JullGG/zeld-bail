...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleEncryptNotification = async (node) => {
        ...
        if (from === WABinary_1.S_WHATSAPP_NET) {
            ...
            const result = await (0, Utils_1.handleIdentityChange)(node, {
                ...
                validateSession: signalRepository.validateSession?.bind?.(signalRepository) || (async () => ({ exists: false })),
                assertSessions,
                debounceCache: identityAssertDebounce,
                logger,
                onBeforeSessionRefresh: async (jid) => {
                    if (messageRetryManager) {
                        const state = await signalRepository.validateSession?.bind?.(signalRepository)?.(jid);
                        if (state?.exists && typeof signalRepository.deleteSession === 'function') {
                            logger.debug({ jid }, 'clearing old session before identity refresh');
    ...
    const sendMessagesAgain = async (key, ids, retryNode, receiptNode) => {
        ...
        const msgs = await Promise.all(ids.map(async (id) => {
            let msg = await getMessage({ ...key, id });
            if (!msg && messageRetryManager) {
                const cached = messageRetryManager.getRecentMessage(remoteJid, id);
                if (cached?.message) {
                    msg = cached.message;
                    logger.debug({ jid: remoteJid, id }, 'using Renvy retry cache for message resend');
                }
            }
            return msg;
        }));
        ...
        if (messageRetryManager?.isMacError?.(retryErrorCode) && typeof signalRepository.deleteSession === 'function') {
            logger.info({ participant, retryErrorCode }, 'MAC retry receipt detected, deleting stale session before resend');
        ...
        if (!injectedFromBundle) {
            ...
            if (typeof receivedRegId === 'number' && Number.isInteger(receivedRegId) && typeof signalRepository.getSessionInfo === 'function') {
                ...
                if (info && info.registrationId !== 0 && info.registrationId !== receivedRegId) {
                    logger.info({ participant, stored: info.registrationId, received: receivedRegId }, 'registration id mismatch on retry, deleting stale session');
                    if (typeof signalRepository.deleteSession === 'function') {
                        await signalRepository.deleteSession([participant]);
                    }
                    else {
        ...
        logger.debug({ participant, sendToAll, injectedFromBundle }, 'prepared session for retry receipt');
        ...
        try {
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (messageRetryManager && attrs.type !== 'retry') {
                        for (const id of ids) {
            ...
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        if (config.renvyResolveLidParticipant === true && (0, WABinary_1.isJidGroup)(msg.key.remoteJid) && ((_f = (_e = (_d = (_c = msg.message) === null || _c === void 0 ? void 0 : _c.extendedTextMessage) === null || _d === void 0 ? void 0 : _d.contextInfo) === null || _e === void 0 ? void 0 : _e.participant) === null || _f === void 0 ? void 0 : _f.endsWith('@lid'))) {
                            if (msg.message.extendedTextMessage.contextInfo) {
                                try {
                                    const metadata = await Promise.race([
                                        groupMetadata(msg.key.remoteJid),
                                        (0, Utils_1.delay)(120).then(() => null)
                                    ]);
    ...
    try { ws?.on?.('close', () => identityAssertDebounce.close?.()); } catch (_) {}
    return {
        ...sock,
        sendMessageAck,
        sendRetryRequest,
