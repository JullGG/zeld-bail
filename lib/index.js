...
if (!global.__RENVY_BAIL_ANTI_CRASH_HANDLER__) {
  ...
  process.on('unhandledRejection', (reason) => {
    ...
    if (isKnownRenvySocketError(reason)) {
      console.log('[ZELD BAIL ANTI-CRASH] Handled promise error:', text.split('\n')[0]);
      return;
    }
    console.log('[ZELD BAIL WARNING] Unhandled rejection:', text);
  });
  process.on('uncaughtException', (err) => {
    const text = safeErrorText(err);
    if (isKnownRenvySocketError(err)) {
      console.log('[ZELD BAIL ANTI-CRASH] Handled exception:', text.split('\n')[0]);
      return;
    }
    console.log('[ZELD BAIL WARNING] Uncaught exception:', text);
  });
}
...
const renderBoot = (done = false) => {
  ...
  if (done) {
    console.log(chalk.greenBright.bold("  ✔ Zeld Bail Ready"));
