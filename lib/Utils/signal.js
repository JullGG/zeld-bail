...
const extractDeviceJids = (result, myJid, myLid, excludeZeroDevices) => {
    // Backward compatible: old call was (result, myJid, excludeZeroDevices)
    if (typeof myLid === 'boolean' && excludeZeroDevices === undefined) {
        excludeZeroDevices = myLid;
        myLid = undefined;
    }
    const { user: myUser, device: myDevice } = WABinary_1.jidDecode(myJid) || {};
    const { user: myLidUser } = myLid ? (WABinary_1.jidDecode(myLid) || {}) : {};
    const extracted = [];
    for (const userResult of result || []) {
        ...
        if (!Array.isArray(deviceList) || !user)
            continue;
        for (const { id: device, keyIndex } of deviceList) {
            if ((!excludeZeroDevices || device !== 0) &&
                ((myUser !== user && myLidUser !== user) || myDevice !== device) &&
                (device === 0 || !!keyIndex)) {
                extracted.push({
                    user,
                    device,
