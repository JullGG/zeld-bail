...
/**
 * Extract an E2E session bundle from an incoming retry receipt.
 * Ported from upstream Baileys 7.x to recover stale/corrupt Signal sessions
 * that often surface as libsignal Bad MAC errors.
 */
const extractE2ESessionFromRetryReceipt = (receipt) => {
    ...
    if (preKeyNode) {
        ...
        preKey = {
            keyId: preKeyId,
            publicKey: (0, crypto_1.generateSignalPubKey)(preKeyPub)
        };
    }
    return {
        registrationId,
