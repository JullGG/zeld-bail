...
const makeEventBuffer = (logger) => {
    ...
    const RENVY_HISTORY_CACHE_MAX = Number(process.env.RENVY_HISTORY_CACHE_MAX || 15000);
    const RENVY_HISTORY_CACHE_CLEAR_INTERVAL = Number(process.env.RENVY_HISTORY_CACHE_CLEAR_INTERVAL || 30 * 60 * 1000);
    const historyCacheCleaner = setInterval(() => {
        if (historyCache.size > RENVY_HISTORY_CACHE_MAX) {
            logger?.trace?.({ size: historyCache.size }, 'Renvy auto cleared message history cache');
            historyCache.clear();
        }
    }, RENVY_HISTORY_CACHE_CLEAR_INTERVAL);
    historyCacheCleaner.unref?.();
