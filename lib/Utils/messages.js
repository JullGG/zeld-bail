...
const generateWAMessageContent = async (message, options) => {
    ...
    // Renvy Bail Fast Plus: scheduledCallCreationMessage helper
    else if ('call' in message && message.call) {
        m.scheduledCallCreationMessage = {
            scheduledTimestampMs: message.call.time || message.call.scheduledTimestampMs || Date.now(),
            callType: message.call.type || message.call.callType || 1,
            title: message.call.title || ''
        };
    }
    // Renvy Bail Fast Plus: ptvMessage / video bulat
    else if ('ptv' in message && message.ptv) {
        const prepared = await (0, exports.prepareWAMessageMedia)(message, options);
        m.ptvMessage = prepared.ptvMessage || prepared.videoMessage;
    }
        ...
        message.poll.toAnnouncementGroup = message.poll.toAnnouncementGroup || false;
        ...
        if (message.poll.forceV2 || message.poll.toAnnouncementGroup) {
            m.pollCreationMessageV2 = pollCreationMessage;
        }
        else if (message.poll.forceV3 || message.poll.selectableCount === 1) {
    ...
    // Renvy Bail Fast Plus: eventMessage helper
    ...
    // Renvy Bail Fast Plus: album parent message helper
    else if ('album' in message && Array.isArray(message.album)) {
        const imageMessages = message.album.filter(item => item && 'image' in item);
        const videoMessages = message.album.filter(item => item && 'video' in item);
    ...
    // Renvy Bail Fast Plus: native interactiveButtons langsung dari sendMessage
    if ('interactiveButtons' in message && !!message.interactiveButtons) {
        const interactiveMessage = {
            nativeFlowMessage: WAProto_1.proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                buttons: message.interactiveButtons,
                messageParamsJson: message.messageParamsJson || message.paramsJson || ''
            })
        };
        if ('text' in message) {
        ...
        if (('title' in message && !!message.title) || ('caption' in message && Object.keys(m).length)) {
            interactiveMessage.header = {
                title: message.title || '',
                subtitle: message.subtitle || '',
                hasMediaAttachment: !!message.media || !!message.image || !!message.video || !!message.document,
            };
            if (Object.keys(m).length && !m.extendedTextMessage) Object.assign(interactiveMessage.header, m);
        }
        else {
    ...
    // Renvy Bail v13: shopStorefrontMessage helper dinonaktifkan karena sering tampil 'update WhatsApp'.
