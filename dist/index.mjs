...
const SHA256_LEN = 32;
const normalizeBaileys = (mod) => {
    const cjs = mod?.default && typeof mod.default === "object" ? mod.default : {};
    const socketFn =
        typeof mod?.default === "function" ? mod.default :
            typeof mod?.default?.default === "function" ? mod.default.default :
                typeof mod?.default?.makeWASocket === "function" ? mod.default.makeWASocket :
                    typeof mod?.makeWASocket === "function" ? mod.makeWASocket :
                        null;
    const merged = { ...cjs, ...mod };
    if (socketFn) {
        merged.default = socketFn;
        merged.makeWASocket = socketFn;
    }
    return merged;
};
const loadBaileys = async () => {
    try {
        return normalizeBaileys(await import("baileys"));
    }
    catch {
        try {
            return normalizeBaileys(await import("@whiskeysockets/baileys"));
        }
        catch {
            throw new Error("Could not import Renvy Baileys. Install the bundled Renvy baileys package.");
        }
    }
};
const toBareJid = (jid) => {
...
export class ActiveCall extends EventEmitter {
    ...
    get state() { return this.#state; }
        ...
        // Renvy fix: if WASM/server does not emit an ended state, resolve locally
        // so the next call does not get stuck as "A call is already active".
...
export class VoipClient {
    ...
    connect = async () => {
        ...
        const makeSocket =
            typeof makeWASocket === "function" ? makeWASocket :
                typeof this.#baileys.makeWASocket === "function" ? this.#baileys.makeWASocket :
                    typeof this.#baileys.default?.default === "function" ? this.#baileys.default.default :
                        typeof this.#baileys.default?.makeWASocket === "function" ? this.#baileys.default.makeWASocket :
                            null;
        if (typeof makeSocket !== "function")
            throw new Error("makeSocket is not a function. Renvy Bail export tidak cocok.");
    ...
    call = async (phoneNumber, opts = {}) => {
        ...
        const resolvedPeer = await this.#signaling.resolveLid(targetPnJid);
        const peerLid = resolvedPeer || targetPnJid;
        ...
        // Renvy fix: always release internal active-call reference on end/error/timeout.
        call.once("ended", () => {
