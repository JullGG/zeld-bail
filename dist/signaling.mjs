...
const loadBaileys = async () => {
    ...
    try {
        _baileysModule = normalizeBaileys(await import("baileys"));
        return _baileysModule;
    }
    catch {
        try {
            _baileysModule = normalizeBaileys(await import("@whiskeysockets/baileys"));
            return _baileysModule;
        }
        catch {
            throw new Error("Could not import Baileys. Install either @whiskeysockets/baileys or the Renvy baileys package.");
        }
    }
};
...
export class SignalingBridge {
    ...
    discoverPeerDevices = async (peerLidJid) => {
        ...
        const jids = devices
            .map((d) => d.jid || (d.user ? this.#baileys.jidEncode(d.user, server, d.device) : ""))
            .filter(Boolean);
        return this.#normalizeStartCallPeerList(jids);
    };
    ...
    resolveLid = async (pnJid) => {
        const normalized = this.#toBareJid(pnJid);
        const fromSocket = await this.#sock.getLIDForPN?.(normalized);
        if (fromSocket)
            return fromSocket;
        const fromRepository = await this.#sock.signalRepository.lidMapping?.getLIDForPN?.(normalized);
    ...
    #maybeDecryptEnc = async (voipNode, peerJid) => {
        ...
        for (const jid of [...new Set(jids.filter(Boolean))]) {
            ...
            if (!refresh && cachedAt && Date.now() - cachedAt < SESSION_CACHE_TTL_MS)
                continue;
            if (!refresh) {
                const validation = this.#sock.signalRepository.validateSession
                    ? await this.#sock.signalRepository.validateSession(jid)
                    : { exists: !!(await this.#sock.authState.keys.get("session", [signalId]))[signalId] };
